...
class IndicatorMixin:
    ...
    @staticmethod
    def p_true_range(high, low, prev_close, up_first):
        denom = pd.Series(np.where(up_first, high, low), index=high.index)
        tr1 = (high - low) / denom
        tr2 = (high - prev_close).abs() / prev_close
        tr3 = (low - prev_close).abs() / prev_close
        true_range = pd.DataFrame(data={"tr1": tr1, "tr2": tr2, "tr3": tr3}).max(axis=1)
        return true_range
