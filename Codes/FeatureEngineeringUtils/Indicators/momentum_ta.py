...
class RSIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        down_direction = -1 * diff.where(diff < 0, 0.0)
        min_periods = 0 if self._fillna else self._window
        self._emaup = up_direction.ewm(alpha=1 / self._window, min_periods=min_periods, adjust=False).mean()
        self._emadn = down_direction.ewm(alpha=1 / self._window, min_periods=min_periods, adjust=False).mean()
        relative_strength = self._emaup / self._emadn
        self._rsi = pd.Series(
            np.where(self._emadn == 0, 100, 100 - (100 / (1 + relative_strength))),
            index=self._close.index,
        )

    def emaup(self) -> pd.Series:
        emaup = self._check_fillna(self._emaup, value=50)
        return pd.Series(emaup, name="emaup")

    def emadn(self) -> pd.Series:
        emadn = self._check_fillna(self._emadn, value=50)
        return pd.Series(emadn, name="emadn")
...
class TSIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._pc_ema = diff_close.ewm(span=self._window_slow, min_periods=min_periods_r, adjust=False).mean()
        self._pc_dema = self._pc_ema.ewm(span=self._window_fast, min_periods=min_periods_s, adjust=False).mean()

        self._apc_ema = abs(diff_close).ewm(span=self._window_slow, min_periods=min_periods_r, adjust=False).mean()
        self._apc_dema = self._apc_ema.ewm(span=self._window_fast, min_periods=min_periods_s, adjust=False).mean()

        self._tsi = 100 * self._pc_dema / self._apc_dema

    def pc_ema(self) -> pd.Series:
        pc_ema = self._check_fillna(self._pc_ema, value=0)
        return pd.Series(pc_ema, name="pc_ema")

    def pc_dema(self) -> pd.Series:
        pc_dema = self._check_fillna(self._pc_dema, value=0)
        return pd.Series(pc_dema, name="pc_dema")

    def apc_ema(self) -> pd.Series:
        apc_ema = self._check_fillna(self._apc_ema, value=0)
        return pd.Series(apc_ema, name="apc_ema")

    def apc_dema(self) -> pd.Series:
        apc_dema = self._check_fillna(self._apc_dema, value=0)
        return pd.Series(apc_dema, name="apc_dema")
...
class UltimateOscillator(IndicatorMixin):
    ...
    def _run(self):
        ...
        self._uo = 100.0 * ((self._weight1 * avg_s) + (self._weight2 * avg_m) + (self._weight3 * avg_l)) / (
                self._weight1 + self._weight2 + self._weight3)

    def bp(self) -> pd.Series:
        bp = self._check_fillna(self._bp, value=-1)
        return pd.Series(bp, name="bp")
...
class KAMAIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        smoothing_constant = (
                (efficiency_ratio * (2.0 / (self._pow1 + 1) - 2.0 / (self._pow2 + 1.0)) + 2 / (self._pow2 + 1.0))
                ** 2.0
        ).values
    ...
    def c2c(self) -> pd.Series:
        c2c = self._check_fillna(self._c2c, value=-1)
        return pd.Series(c2c, name="c2c")

    def kama(self) -> pd.Series:
        kama_series = pd.Series(self._kama, index=self._close.index)
        kama = self._check_fillna(kama_series, value=-1)
        return pd.Series(kama, name="kama")

    def skama(self) -> pd.Series:
        kama_series = pd.Series(self._kama, index=self._close.index)
        kama_series = self._check_fillna(kama_series, value=-1)
        scaled = (kama_series - self._open) / self._open
        return pd.Series(scaled, name="skama")
...
class ROCIndicator(IndicatorMixin):
    ...
    def _run(self):
        denom = self._close.shift(self._window)
        denom = denom.replace([0], 0.001)
        self._roc = 100 * (self._close - denom) / denom
...
class AwesomeOscillatorIndicator(IndicatorMixin):
    ...
    def median_price(self) -> pd.Series:
        _med = self._check_fillna(self._med, value=0)
        return pd.Series(_med, name="med")
...
class StochRSIIndicator(IndicatorMixin):
    ...
    def _run(self):
        ...
        highest_high_rsi = self._rsi.rolling(self._window).max()
        self._stochrsi = (self._rsi - lowest_low_rsi) / (highest_high_rsi - lowest_low_rsi)
...
class logKAMAIndicator(IndicatorMixin):
    """Kaufman's Adaptive Moving Average (KAMA)
...
class logAwesomeOscillatorIndicator(IndicatorMixin):
    """Awesome Oscillator
