...
from Codes.FeatureEngineeringUtils.Indicators.volume_ta import (
    ...
    logVolumeWeightedAveragePrice,
    Volume_ema
)


def add_log_volume_ta(df0: pd.DataFrame, opn: str, high: str, low: str, close: str, volume: str, fillna=False,
                  colprefix=""):
    df = {}
    log_open = df0[opn].apply(np.log)
    log_open = log_open - log_open.shift(1).fillna(method='backfill')
    log_high = df0[high].apply(np.log)
    log_high = log_high - log_high.shift(1).fillna(method='backfill')
    log_low = df0[low].apply(np.log)
    log_low = log_low - log_low.shift(1).fillna(method='backfill')
    log_close = df0[close].apply(np.log)
    log_close = log_close - log_close.shift(1).fillna(method='backfill')
    log_vol = df0[volume].apply(np.log)
    log_vol = log_vol - log_vol.shift(1).fillna(method='backfill')

    df[f"{colprefix}volume_log_volume"] = log_vol

    # Accumulation Distribution Index
    df[f"{colprefix}volume_adi"] = AccDistIndexIndicator(
        high=log_high, low=log_low, close=log_close, volume=log_vol, fillna=fillna
    ).acc_dist_index()

    # On Balance Volume
    df[f"{colprefix}volume_obv"] = OnBalanceVolumeIndicator(close=log_close, volume=log_vol, fillna=fillna
                                                            ).on_balance_volume()
    ...
    # Force Index
    indicator_fi = ForceIndexIndicator(close=log_close, volume=log_vol, w1=13, w2=7, fillna=fillna)
    df[f"{colprefix}volume_fi_13"] = indicator_fi.force_index()
    df[f"{colprefix}volume_fi_7"] = indicator_fi.force_index2()

    # Money Flow Indicator
    indicator_mfi = MFIIndicator(high=log_high, low=log_low, close=log_close, volume=log_vol, w1=14, w2=10,
                                 fillna=fillna)
    ...
    # Ease of Movement
    indicator_eom = EaseOfMovementIndicator(high=log_high, low=log_low, volume=log_vol, window=14, fillna=fillna)
    ...
    df[f"{colprefix}volume_nvi"] = NegativeVolumeIndexIndicator(
        close=log_close, volume=log_vol, fillna=fillna
    ).negative_volume_index()

    # Volume Weighted Average Price
    indicator_vwap = logVolumeWeightedAveragePrice(
        opn=log_open,
        high=log_high,
        low=log_low,
        close=log_close,
...
def add_log_volatility_ta(df0, opn: str, high: str, low: str, close: str, up_first='None', fillna=False, colprefix=""):
    df = {}
    ...
    df[f"{colprefix}volatility_atr"] = AverageTrueRange(
        close=log_close, high=log_high, low=log_low, window=10, fillna=fillna
    ).average_true_range()
    ...
    # Bollinger Bands
    indicator_bb = logBollingerBands(
        close=log_close, opn=log_open, window=20, window_dev=2, fillna=fillna
    )
    ...
    indicator_kc = logKeltnerChannel(close=log_close, high=log_high, low=log_low, opn=log_open, window=10)
    ...
    # Donchian Channel
    indicator_dc = logDonchianChannel(
        high=log_high, low=log_low, close=log_close, opn=log_open, window=20, offset=0, fillna=fillna
    )
    ...
    # Ulcer Index
    ui = logUlcerIndex(close=log_close, window=14, fillna=fillna)
...
def add_log_momentum_ta(df0, opn: str, high: str, low: str, close: str, volume='None', fillna=False, colprefix: str = ""):
    df = {}
    ...
    # Stoch RSI (StochRSI)
    indicator_srsi = StochRSIIndicator(close=log_close, window=14, smooth1=3, smooth2=3, fillna=fillna)
    df[f"{colprefix}momentum_stoch_rsi"] = indicator_srsi.stochrsi()

    # TSI Indicator
    indicator_tsi = TSIIndicator(close=log_close, window_slow=25, window_fast=13, fillna=fillna)
    ...
    # Ultimate Oscillator
    uo = UltimateOscillator(high=log_high, low=log_low, close=log_close, window1=7, window2=14, window3=28,
                            weight1=4.0,
    ...
    # Awesome Oscillator
    indicator_ao = logAwesomeOscillatorIndicator(high=log_high, low=log_low, opn=log_open, window1=5, window2=34)
    df[f"{colprefix}momentum_med"] = indicator_ao.median_price()
    df[f"{colprefix}momentum_ao"] = indicator_ao.awesome_oscillator()

    # KAMA
    indicator_kama = logKAMAIndicator(close=log_close, opn=log_open, window=10, pow1=2, pow2=30)
    ...
    # Rate Of Change
    df[f"{colprefix}momentum_roc"] = ROCIndicator(close=log_close, window=12, fillna=fillna).roc()

    # Percentage Volume Oscillator
    if volume != 'None':
        indicator_pvo = PercentageVolumeOscillator(volume=log_vol, window_slow=26, window_fast=12,
                                                   window_sign=9)
...
def add_log_trend_ta(df0, opn: str, high: str, low: str, close: str, fillna=False, colprefix: str = ""):
    df = {}
    ...
    # SMAs
    df[f"{colprefix}trend_sma_12"] = logSMAIndicator(close=log_close, opn=log_open, window=12).sma_indicator()
    df[f"{colprefix}trend_sma_26"] = logSMAIndicator(close=log_close, opn=log_open, window=26).sma_indicator()
    df[f"{colprefix}trend_sma_50"] = logSMAIndicator(close=log_close, opn=log_open, window=50).sma_indicator()

    # EMAs
    ema_10 = logEMAIndicator(close=df0[close], opn=log_close, window=10, fillna=fillna)
    df[f"{colprefix}trend_ema_10"] = ema_10.ema_indicator()
    df[f"{colprefix}trend_ns_ema_10"] = ema_10.ns_ema_indicator()

    ema_12 = logEMAIndicator(close=df0[close], opn=log_close, window=12, fillna=fillna)
    df[f"{colprefix}trend_ema_12"] = ema_12.ema_indicator()
    df[f"{colprefix}trend_ns_ema_12"] = ema_12.ns_ema_indicator()

    ema_15 = logEMAIndicator(close=df0[close], opn=log_close, window=15, fillna=fillna)
    df[f"{colprefix}trend_ema_15"] = ema_15.ema_indicator()
    df[f"{colprefix}trend_ns_ema_15"] = ema_15.ns_ema_indicator()

    ema_20 = logEMAIndicator(close=df0[close], opn=log_close, window=20, fillna=fillna)
    df[f"{colprefix}trend_ema_20"] = ema_20.ema_indicator()
    df[f"{colprefix}trend_ns_ema_20"] = ema_20.ns_ema_indicator()

    ema_23 = logEMAIndicator(close=log_close, opn=log_open, window=23, fillna=fillna)
    df[f"{colprefix}trend_ema_23"] = ema_23.ema_indicator()
    df[f"{colprefix}trend_ns_ema_23"] = ema_23.ns_ema_indicator()

    ema_26 = logEMAIndicator(close=log_close, opn=log_open, window=26, fillna=fillna)
    df[f"{colprefix}trend_ema_26"] = ema_26.ema_indicator()
    df[f"{colprefix}trend_ns_ema_26"] = ema_26.ns_ema_indicator()

    ema_50 = logEMAIndicator(close=log_close, opn=log_open, window=50, fillna=fillna)
    df[f"{colprefix}trend_ema_50"] = ema_50.ema_indicator()
    df[f"{colprefix}trend_ns_ema_50"] = ema_50.ns_ema_indicator()

    # MACD
    indicator_macd = logMACD(close=log_close, opn=log_open, window_slow=26, window_fast=12, window_sign=9, fillna=fillna)
    ...
    indicator_adx = ADXIndicator(
        high=log_high, low=log_low, close=log_close, window=14, fillna=fillna
    )
    ...
    # TRIX Indicator
    indicator_trix = logTRIXIndicator(close=df0[close], opn=log_close, window=15, fillna=fillna)
    ...
    # Mass Index
    indicator_mass_index = MassIndex(high=log_high, low=log_low, window_fast=9, window_slow=25, fillna=fillna)
    ...
    # CCI Indicator
    indicator_cci = CCIIndicator(high=log_high, low=log_low, close=log_close, window=20, constant=0.015,
                                 fillna=fillna)
    df[f"{colprefix}trend_typical"] = indicator_cci.tp()
    df[f"{colprefix}trend_cci"] = indicator_cci.cci()

    # DPO Indicator
    df[f"{colprefix}trend_dpo"] = DPOIndicator(close=log_close, window=20, fillna=fillna).dpo()
    ...
    # Ichimoku Indicator
    indicator_ichi = logIchimokuIndicator(high=log_high, low=log_low, opn=log_open, window1=9, window2=26, window3=52)
    ...
    # Aroon Indicator
    indicator_aroon = AroonIndicator(close=log_close, window=25, fillna=fillna)
    df[f"{colprefix}trend_aroon_up"] = indicator_aroon.aroon_up()
    df[f"{colprefix}trend_aroon_down"] = indicator_aroon.aroon_down()
    indicator_aroon = AroonIndicator(close=log_high, window=25, fillna=fillna)
    df[f"{colprefix}trend_high_aroon_up"] = indicator_aroon.aroon_up()
    indicator_aroon = AroonIndicator(close=log_low, window=25, fillna=fillna)
    df[f"{colprefix}trend_low_aroon_down"] = indicator_aroon.aroon_down()

    # PSAR Indicator
    indicator_psar = logPSARIndicator(high=log_high, low=log_low, close=log_close, opn=log_open, step=0.02,
                                   max_step=0.20)
    ...
    indicator_stc = STCIndicator(close=log_close, window_slow=50, window_fast=23, cycle=10, smooth1=3, smooth2=3)
...
def add_all_log_stationary_ta_features(df, opn: str, high: str, low: str, close: str, volume='None', up_first='None',
    ...
    """
    if volume != 'None':
        df = add_log_volume_ta(df0=df, opn=opn, high=high, low=low, close=close, volume=volume, fillna=fillna,
                           colprefix=colprefix)
    df = add_log_volatility_ta(df0=df, opn=opn, high=high, low=low, close=close, up_first=up_first, fillna=fillna,
                           colprefix=colprefix)
    df = add_log_momentum_ta(df0=df, opn=opn, high=high, low=low, close=close, volume=volume, fillna=fillna,
                         colprefix=colprefix)
    df = add_log_trend_ta(df0=df, opn=opn, high=high, low=low, close=close, fillna=fillna, colprefix=colprefix)
    return df
